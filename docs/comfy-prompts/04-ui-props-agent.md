...
**Western split (DEC-026):** UI chrome may keep a **light western wood/brass** read — that is **character-side UI**, not cavern scenery. Do not let porch/steeple/hitching-post language leak into the 3D colony. In-world props that *are* the cavern (lanterns, antenna, lift-adjacent masts, string lights) are Lykos industrial; hotspot **building fronts** are utilitarian stacked colony architecture, not a western main street. Screens and HUD layout this art must serve: `docs/ui-wireframes.html` (S1–S7) and `docs/PRD.md` §10.
...
**Hotspot building fronts (items 25–27)** — texture/reference for Unity **Lykos stacked-block** hotspot meshes (PRD R-10 names unchanged; R-15 / DEC-026 look). Deliver front-on, flat-lit, 1024²: utilitarian cubic colony architecture, flat roofs, small window/lantern points, dusty beige/grey. **Not** weathered saloon grandeur, **not** a chapel steeple, **not** a ranch house + porch.
...
- **② FRAMING facade variant** (items 25–27, with both BG nodes set to `(the weathered building surface fills the entire frame:1.3)`): `flat front-on orthogonal elevation view of a utilitarian stacked-block Lykos colony building, game environment texture reference art, the facade fills the frame edge to edge, evenly flat lit, cubic flat-roofed Martian underground architecture, monochromatic burnt-sienna palette, warm white-gold window glow accents, matte painterly semi-realism, muted matte dusty surfaces, no text, no watermark`. Facades render with painterly sky behind — delivered as the BiRefNet cutout (RGBA), which removes the sky and satisfies no-sky. **Do not prompt saloon grandeur, steeples, or ranch porches.**
