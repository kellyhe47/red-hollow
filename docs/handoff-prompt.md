...
- **Art arrives asynchronously** from four ComfyUI pipelines (`docs/comfy-prompts/`). All visuals load via swappable references; the game must be fully playable with primitive placeholders (capsules/cubes/flat color). No code path may block on an asset existing.
